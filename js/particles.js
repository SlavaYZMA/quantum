...
window.updateParticles = (sketch) => {
    ...
    window.particles.forEach((p, i) => {
        try {
            ...
            if (window.currentStep === 4 && mouseX > 0 && mouseX < 400 && mouseY > 0 && mouseY < 400) {
            ...
            if (i < 5) { // Ограничим логирование первыми 5 частицами для читаемости
    ...
    window.particles.forEach((p, i) => {
        try {
            ...
            if (distance < window.mouseInfluenceRadius && distance > 0) {
                ...
                state.probability = Math.max(0.1, state.probability - 0.01);
                state.a = Math.floor(state.probability * 255);
                console.log(`Particle ${i} affected by mouse, new alpha: ${state.a}`);
            }
        } catch (error) {
            console.error(`Error in observeParticles for particle ${i}:`, error);
        }
    });
};
